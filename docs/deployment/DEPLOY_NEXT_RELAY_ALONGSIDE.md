...
- **没有成员资格校验**：`main.py` 没启用 `membership` / `invite_registry`，任何
  ...
  接受，**不要对外公开这个地址**。
- **设备注册表已持久化**：默认写到 `server/relay/devices.db`。这不只是省去重连——
  内存态的话每次重启 TOFU 保护都被重置一次，攻击者等一次重启就能抢注别人的
  deviceId。这个文件要和代码一起备份；删掉它等于让所有设备重新走首次注册。
  临时联调可以设 `SECUREINCHAT_DEVICE_DB=":memory:"` 退回旧行为。
- **已有限流**：认证前按来源限连接尝试（30 秒 10 次），认证后按设备限消息速率
  （每秒 30 条，心跳不计）。正常使用碰不到这些阈值。
